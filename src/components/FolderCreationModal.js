...
export default class FileCreationModal extends React.Component {
  ...
  handleSubmit = async () => {
    if (this.props.doAddFolder)
      this.props.doAddFolder(this.state.name);
    this.toggle();
  }
  ...
  render() {
    return (
      <>
        <Button style={{ width: "10rem" }} size="sm" variant="dark" onClick={this.toggle}>Créer un dossier</Button>
        <Modal show={this.state.show} onHide={this.toggle} animation={false}>
          ...
          <Modal.Body>
            <Form.Group>
              <Form.Control name="name" type="text" size="sm" placeholder="Nom dossier" value={this.state.name ? this.state.bame : ""} onChange={this.onTextChange} />
            </Form.Group>
          </Modal.Body>
          <Modal.Footer>
